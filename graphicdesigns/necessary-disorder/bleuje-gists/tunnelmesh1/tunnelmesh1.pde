// note : defining the subset of all triangles in setup() takes quite a lot of time
...
class Dot{
  //float r = random(400,600)*0.9;
  float r = random(480,520)*0.9;
  ...
  float rf = random(-1,1)*7;
  
  float sz = random(4,10);
  
  PVector pos(float p){
    p = (p+offset)%1;
    
    float z = map(p,0,1,-7000,1000);
    
    float rot = -5.0*pow(1-p,3.0)+rf*p;
    
    float xx = x*cos(rot) - y*sin(rot);
    float yy = x*sin(rot) + y*cos(rot);
    
    return new PVector(xx+22000*pow(1-p,4.0)-50,-(yy+22000*pow(1-p,4.0)-50),z);
  }
}
...
class Triangle{
  ...
  void show(float p){
    ...
    if(max(max(dist1,dist3),dist2)<=DMAX){
      ...
      //stroke(200);
      strokeWeight(0.7);
      //fill(lerpColor(colorArray[type],#dddddd,-0.2));
      fill(15);
