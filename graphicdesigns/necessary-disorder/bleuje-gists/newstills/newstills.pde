...
class Thing{
  ...
  float r = 0.3*width*pow(random(1),1.5);
  ...
  void show(float t){
    for(int i=0;i<np;i++){
      ...
      float rot = rot0 + 2.75*(float)noise.eval(seed + 5.0*p,0);
      rotate(rot);
...
void draw(){
  ...
  float t = 1.0*frameCount/numDraw;
  
  println(frameCount,"/",numDraw);
  ...
  if(frameCount == numDraw){
    saveFrame("pic" + random(100000) + ".png");
    stop();
    println("finished");
  }
