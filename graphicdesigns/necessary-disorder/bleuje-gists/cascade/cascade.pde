...
PVector position2(float p,float q){ 
    float pp = 5*p;
    float z = 600-200*(floor(pp)+ease(pp-floor(pp),2.5)) + p*1700*pow(q-0.5,2.0);
    float y = map(p,0,1,-2.5*width,width);
    float x = 1.3*pow(p,2.0)*map(q,0,1,-1.3*width,1.3*width);
...
class Thing{
  ...
  float coladd(float p){
    return 200*pow(map((float)noise.eval(2*seed+11.5*p,0),-1,1,0,1),2.0);
  }
  ...
  void show(){
    for(int k=0;k<=K;k++){
      ...
      n.normalize();
      float d = 75;
      v0.add(n.copy().mult(-d)).add(displacement(p0)).add(new PVector(-100*map(q0,0,1,1,-1),0,0));
...
void draw_(){
  ...
  translate(width/2,height/2,-0.0*width);
