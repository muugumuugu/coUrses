...
void draw_(){
  ...
  for(int i=0;i<n;i++){
    float theta01 = TWO_PI*(i+1*N*t)/n;
    float theta02 = TWO_PI*(i+1+1*N*t)/n;
    
    noStroke();
    
    beginShape(TRIANGLE_STRIP);
    for(int j=0;j<m;j++){
      float theta1 = theta01 + 5*TWO_PI*j/m;
      float theta2 = theta02 + 5*TWO_PI*j/m;
      ...
      float dr = 50*(float)noise.eval(123+0.001*Z+rad*cos(TWO_PI*(-t+offset)),rad*sin(TWO_PI*(-t+offset)));
