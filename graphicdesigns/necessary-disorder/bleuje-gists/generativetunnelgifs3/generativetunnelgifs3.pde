...
class Patch{
  ...
  float lx = 0.57*pow(random(0,1),2.0);
  //float ly = 0.17*pow(random(0,1),2.0);
  float ly = lx;
  
  int m = floor(random(50,175));
...
public void draw3D(PVector p1, PVector p2, float par) {
    ...
    vec3d.add(new PVector(0,0,20*par-8));
    //vec3d.mult(par);
    ...
    if(-vec3d.z+zdist>0){
      ...
      strokeWeight(min(3,random(0,1)*2.5f*proj.z));
      stroke(255,37*sin(par));
...
public PVector project(PVector vec){
  float x = 100*vec.x/(-vec.z+zdist);
  float y = 100*vec.y/(-vec.z+zdist);
