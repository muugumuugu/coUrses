...
class Thing{
  ...
  float x(float ph){
    return cx + map(noise(100+id+rad*cos(TWO_PI*(t+ph)),rad*sin(TWO_PI*(t+ph))),0,1,-mov,mov);
  }
  
  float y(float ph){
    return cy + map(noise(1000+100+id+rad*cos(TWO_PI*(t+ph)),rad*sin(TWO_PI*(t+ph))),0,1,-mov,mov);
  }
