...
PVector position2(float p,float theta){
    ...
    float z = -softplus(-pp,smth)+150*p;
...
class Thing{
  ...
  float coladd(float p){
    return 25+300*pow(map((float)noise.eval(2*seed+9.5*p,0),-1,1,0,1),4.0);
  }
  ...
  float dx(float p){
    return displacementI*(float)noise.eval(3*seed+6.0*p,0);
  }
  
  float dy(float p){
    return displacementI*(float)noise.eval(4*seed+6.0*p,0);
  }
  
  float dz(float p){
    return displacementI*(float)noise.eval(5*seed+6.0*p,0);
  }
  
  float dr(float p){
    return ((float)noise.eval(5*seed+3.0*p,0)+1)/2;
  }
