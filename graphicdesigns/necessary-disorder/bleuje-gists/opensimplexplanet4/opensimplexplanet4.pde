...
class Thing{
  ...
  void show(){
    for(int i=0;i<m;i++){
      ...
      //float pp = 2*pow(p,2.0);
      
      float dphi = 0.4*(float)noise.eval(0*seed + 8.0*cos(phi) + rad*cos(TWO_PI*(freq*p-t)),rad*sin(TWO_PI*(freq*p-t)),change*p) + PI*pow(p,1.0);
      float dtheta = 0.1*(float)noise.eval(0*2*seed + 10.0*cos(phi) + rad*cos(TWO_PI*(freq*p-t)),rad*sin(TWO_PI*(freq*p-t)),change*p);
