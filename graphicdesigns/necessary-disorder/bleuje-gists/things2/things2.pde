...
class Thing{
  ...
  int m = floor(random(600,800));
  
  int K = floor(random(4,8));
  
  float sw = random(1.5,2.0);
  ...
  void show(){
    ...
    for(int k=0;k<K;k++){
      for(int i=0;i<=m;i++){
        float p = 1.0*i/m;
        float xx = x()+2*dl*(float)noise.eval(3*seed+rad*cos(TWO_PI*(p+t)),rad*sin(TWO_PI*(p+t)),mr2*cos(TWO_PI*t),mr2*sin(TWO_PI*t)+0.13*k);
        float dy = dl*(float)noise.eval(4*seed+rad*cos(TWO_PI*(p+t)),rad*sin(TWO_PI*(p+t)),mr2*cos(TWO_PI*t),mr2*sin(TWO_PI*t)+0.13*k);
