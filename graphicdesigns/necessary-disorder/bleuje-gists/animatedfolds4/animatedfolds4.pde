...
int randseed = 197;
//Pretty good seeds : 158,161,166,168,175,177,185
/////////////////////////////////////
// set precision to something like 7.0 to check out what it looks like quickly, and set it to 1.0 or less for high quality rendering
float precision = 1.0;
// the stroke alpha
float alphaMax = 7;
// number of rotating white bars
int numberOfParts = 6;
// number of frames in the gif
int numFrames = 100;
// radius where a white bar starts
float rbegin = 1.0;
// radius where a white bar ends
float rend = 3.0;
// rotation speed
int rot = 3;
...
void initialize() {
  ...
  dosinusoidal = false;
  fnumber = (int)floor(random(5,7.99));
...
void draw() {
  ...
  if(go) {
    ...
    float tt = (t+1-0.5)%1;
    tt = ease2(tt,2.0,0.6);
    float theta = map(iPart+rot*tt,0,numberOfParts,0,TWO_PI);
    float s = precision*alphaMax;
    stroke(255,s);
    for(float theta2=theta;theta2<=theta+PI/70;theta2+=precision*0.0005*1400/(2*width)){
...
void drawme(float x, float y,float t) {
...
class Folds {
  ...
  void draw(PVector p,PVector p0,float t) {
    float interp = ease(map(sin(TWO_PI*t - HALF_PI),-1,1,0,1),2.0);
    PVector res = new PVector(lerp(p0.x,p.x,interp),lerp(p0.y,p.y,interp));
    float i = map(res.x,xmin,xmax,screenl,screenr);
    float j = map(res.y,ymin,ymax,screenl,screenr);
...
float ease2(float p, float g, float interp) {
  float res = 0;
  if (p < 0.5) 
    res = 0.5 * pow(2*p, g);
  else
    res = 1 - 0.5 * pow(2*(1 - p), g);
  return lerp(p,res,interp);
}