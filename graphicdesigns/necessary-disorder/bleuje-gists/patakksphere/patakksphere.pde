...
class Path{
  ...
  void show(){
    ...
    for(int i=0;i<npart;i++){
      for(float eps=0;eps<0.05;eps+=0.01){
      // Particle location calculated by linear interpolation from the computed positions
      float loc = constrain(map(i+tt+eps,0,npart,0,len-1),0,len-1-0.001);;
      int i1 = floor(loc);
      int i2 = i1+1;
      ...
      float alpha = fact*255*pow(sin(PI*loc/(len-1)),0.6);
...
PVector field(float x,float y,float z){
  ...
  return new PVector(50 + 20*sin(PI*(x+z)/30),30+30*sin(PI*(x+y)/50),20);
}
