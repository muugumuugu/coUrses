...
class Center{
  ...
  float repulse = random(15,15);
  
  Center(float x_,float y_,float r_){
    x= x_;
    y = y_;
    repulse = r_;
  }
...
PVector field(float x,float y){
  ...
  return new PVector(sumx*random(0.5,1.1),sumy*random(0.5,1.1)+15);
}
...
void setup(){
  ...
  array[0] = new Center(width/2,height/2,15);
...
void draw_(){
  ...
  stroke(255);
  strokeWeight(1);
  ellipse(width/2,height/2,orad*2,orad*2);
