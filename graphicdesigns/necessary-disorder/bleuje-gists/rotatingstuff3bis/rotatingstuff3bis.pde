...
float dr(float p,float off){
  return 70*(float)noise.eval(SEED+2*cos(TWO_PI*p),2*sin(TWO_PI*p),off/3);
}
...
void drawThing(float tt){
  for(int k=0;k<K;k++){
    for(int i=0;i<m;i++){
      ...
      float f = 1-1.0*pow(1.0*i/m,1.8);
      float xx = px(p,0.3*k)*f;
      float yy = py(p,0.3*k)*f;
      
      stroke(255,(i==0?255:8+25*l));
      strokeWeight((i==0?6.5:4.5)*noise(10*k)*(1-1.0*i/m));
      point(xx,yy);
    }
  }
}
