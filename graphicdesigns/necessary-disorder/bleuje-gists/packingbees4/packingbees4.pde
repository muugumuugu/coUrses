...
class Circle{
  ...
  float rr = random(0.3,0.9);
  
  float es = random(1.5,6);
  
  float turns = int(1+random(4));
  
  float arcl = random(0.2,TWO_PI-0.2);
  ...
  Circle(int i){
    ...
    while(!ok){
      ...
      float value = map(dist(x,y,0,0),0,size*width,0,1);
  ...
  void show2(){
    ...
    if(type == 1 || type == 0){
      ...
      push();
      translate(x,y);
      float theta_add = sign*turns*TWO_PI*ease((t+offset)%1,es);
      arc(0,0,2*r,2*r,thetaSin + theta_add,thetaSin + arcl + theta_add);
      
      fill(0);
      //stroke(0);
      noStroke();
      ellipse(0,0,2*r*rr,2*r*rr);
      pop();
    }/*
    if(type == 3){
      //fill(255);
