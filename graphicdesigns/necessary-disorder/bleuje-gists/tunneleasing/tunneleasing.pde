...
class Thing{
  ...
  void show(){
    ...
    float tt_ = (t+toff)%1;
    float tt = (58*lerp(ease(tt_,3.2),tt_,0.55))%1;
    
    for(int i=0;i<m;i++){
      float pp = 1.0*(i+tt)/m;
      float p = pow(pp,2.5);
      float r = 0.75*lerp(0.0,1,p)*width;
      ...
      float y = r*sin(theta2) - 1000*pow(1-pp,2.5);
      
      float sz2 = map((float)noise.eval(0.3*x,0.3*y,0.5*cos(TWO_PI*t),0.5*sin(TWO_PI*t)),-1,1,0.5,1.5);
      
      strokeWeight(1+sz2*sz*3*p);
      
      float z = map(pp,0,1,-2.0*width,0.5*width);
