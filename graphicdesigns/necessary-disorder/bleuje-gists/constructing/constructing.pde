...
PVector position(float r,float p,float theta){
  return new PVector(r*cos(theta+0.9*p),r*sin(theta+0.9*p)-pow((1-p),2.0)*2100+280,-ZMAX*(1-p)-250);
}

class Thing{
  float seed = random(10,1000);
  
  float offset = random(-0.5,1.0);
  
  int i,j;
  
  float r = R-130*pow(random(0.9),5.5);
  float dist = random(500,1500);
  ...
  float param(float p){
    return 1-ease(constrain(2*p-offset,0,1),4.7);
  }
  ...
  void show(){
    for(int k=0;k<K;k++){
      ...
      //stroke(constrain(30+map(v0.z,-ZMAX,0,0,1)*col,0,255));
      stroke((50+1.0*coladd(p0))*(1-q));
      fill(0);
      strokeWeight(1.5);
