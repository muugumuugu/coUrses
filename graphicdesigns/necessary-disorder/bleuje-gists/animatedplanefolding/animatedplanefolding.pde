...
void initialize() {
  ...
  fnumber = (int)floor(random(3,4.99));
...
void draw() { 
  ...
  if(go) {
    ...
    float theta = map(iCurve+t,0,numberOfCurves,0,TWO_PI);
    //float s = alphaMax*pow(sin(PI*(iCurve+t)/numberOfCurves),3.0);
    float s = alphaMax;
    stroke(255,s);
    
    //float step2 = lerp(step,1000*step,pow(1-s/alphaMax,0.75));
    
    for(float r=rmin;r<=rmax;r+=step){
      float x = r*cos(theta);
      float y = r*sin(theta);
      drawme(x,y);
    }
    ...
    if(iCurve == numberOfCurves) {
      ...
      println(iFrame+"/"+numFrames);
       ...
       saveFrame("result"+nameSeed+"-"+String.format("%04d", iFrame)+".png");
