...
class Digit{
  int nd = 2+floor(50*pow(random(0,1),5.0));
...
class Column{
  ...
  float sz = floor(random(0,9)) + map(m,0,40,0,9);
  ...
  float factor = 0.02+0.065*pow(random(0,1),0.75);
  ...
  void show(){
    for(int i=0;i<m;i++){
      ...
      textSize(floor(sz));
      //println(offset);
      fill(255,255*pow(offset,2.5));
      float y = map(i,0,m,-100,height+100) + up*offset;
      
      float p = constrain(y/height,0,1);
      float interp = p*p+0.2;
      
      float xx = lerp(width/2,lerp(x,width/2,0.1),interp);
      
      int ch = digitarray[i].res(t);
      
      String sh = "30A0";
      int nn = unhex(sh);
      char[] chars = Character.toChars(10000+ch);
