...
float depth(float x,float y){
  ...
  return -300+250*(1-sin(PI*p))-150*pow(((cos(TWO_PI*q)+1)/2),1.7);
}
...
void draw_(){
  background(232);
  push();
  translate(width/2,height/2-260,5);
  rotateX(0.28*PI);
  ...
  for(int i=0;i<n*N;i++){
    ...
    for(int j=0;j<m;j++){
      ...
      float dy = 15*(float)noise.eval(2*seed+rad*cos(TWO_PI*(-t+offset)),rad*sin(TWO_PI*(-t+offset)),0.01*xx2);
