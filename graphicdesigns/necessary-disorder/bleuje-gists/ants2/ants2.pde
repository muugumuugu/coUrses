...
class Star{
  ...
  void show(){
    for(int k=0;k<15;k++){
      float p = (t+0.008*k+offset)%1;
      
      //float alpha = 255*pow(sin(PI*p),1.0);
      float alpha = 150;
      
      float dx = 0.7*l*(float)noise.eval(seed+0.03*l*cos(TWO_PI*p),0.03*l*sin(TWO_PI*p));
      float dy = 0.7*l*(float)noise.eval(2*seed+0.03*l*cos(TWO_PI*p),0.03*l*sin(TWO_PI*p));
