...
void draw() { 
  ...
  if (go) {
    ...
    for(float x=-3.0;x<=3.0;x+=lestep/ratio){
      for(float y=-3.0;y<=3.0;y+=lestep*ratio){
        drawme(x, y, t);
      }
    }
    ...
    //  println(theta/TWO_PI);

//if(iFrame<numFrames) 
   // saveFrame("result"+"-"+String.format("%04d", iFrame)+".png");
...
PVector noiseFold(PVector n,float t){
  float x = n.x;
  float y = n.y;
  
  float scale = noise_scale;
  
  return new PVector(x + noisel*(float)noise.eval(scale*x,scale*y,motion_rad*cos(TWO_PI*t),motion_rad*sin(TWO_PI*t)),y + noisel*(float)noise.eval(100+scale*x,scale*y,motion_rad*cos(TWO_PI*t),motion_rad*sin(TWO_PI*t)));
  
}

void drawme(float x, float y,float time_) {
  ...
  PVector interm = noiseFold(n,time_);
  
  PVector n2 = new PVector(0, 0);

  switch(fnumber2) { 
  case 1:
    n2.add( f.getFoldbyNo(folds2[0], interm, 1) );
    break;
  case 2:
    n2.add( f.getFoldbyNo(folds2[0], f.getFoldbyNo(folds2[1], interm, 1), 1) );  
    break;
  case 3:
    n2.add( f.getFoldbyNo(folds2[0], f.getFoldbyNo(folds2[1], f.getFoldbyNo(folds2[2], interm, 1), 1), 1) );  
    break;
  case 4:
    n2.add( f.getFoldbyNo(folds2[0], f.getFoldbyNo(folds2[1], f.getFoldbyNo(folds2[2], f.getFoldbyNo(folds2[3], interm, 1), 1), 1), 1) );  
    break;
  case 5:
    n2.add( f.getFoldbyNo(folds2[0], f.getFoldbyNo(folds2[1], f.getFoldbyNo(folds2[2], f.getFoldbyNo(folds2[3], f.getFoldbyNo(folds2[4], interm, 1), 1), 1), 1), 1) );
    break;
  case 6:
    n2.add( f.getFoldbyNo(folds2[0], f.getFoldbyNo(folds2[1], f.getFoldbyNo(folds2[2], f.getFoldbyNo(folds2[3], f.getFoldbyNo(folds2[4], f.getFoldbyNo(folds2[5], interm, 1), 1), 1), 1), 1), 1) );  
    break;
  case 7:
    n2.add( f.getFoldbyNo(folds2[0], f.getFoldbyNo(folds2[1], f.getFoldbyNo(folds2[2], f.getFoldbyNo(folds2[3], f.getFoldbyNo(folds2[4], f.getFoldbyNo(folds2[5], f.getFoldbyNo(folds2[6], interm, 1), 1), 1), 1), 1), 1), 1) );    
    break;
  case 8:
    n2.add( f.getFoldbyNo(folds2[0], f.getFoldbyNo(folds2[1], f.getFoldbyNo(folds2[2], f.getFoldbyNo(folds2[3], f.getFoldbyNo(folds2[4], f.getFoldbyNo(folds2[5], f.getFoldbyNo(folds2[6], f.getFoldbyNo(folds2[7], interm, 1), 1), 1), 1), 1), 1), 1), 1) );  
    break;
  }
...
final float gdoffs_flip(float a, float b, float c) {return (c*(b-a)+a);}
  ...
  void draw(PVector p) {
    ...
    float i = map(p.x+0.000001 * randomGaussian(),xmin,xmax,screenl,screenr);
    float j = map(p.y+0.000001 * randomGaussian(),ymin,ymax,screenl,screenr);
    point(i,j);
  }
