...
PVector position2(float p,float theta){
    ...
    float x = r*cos(theta+1.6*p);
    float y = r*sin(theta+1.6*p);
...
class Thing{
  ...
  void show(){
    for(int k=0;k<=K;k++){
      ...
      stroke((5+0.7*coladd(p0)+0.8*coladd2(p0))*1.2+(1-p0)*170);
      fill(0);
      strokeWeight(max(1,0.05*pp0));
      ...
      sphere(max(0.25,dr(p0)*0.05*pp0));
      pop();
    }
  }
}
