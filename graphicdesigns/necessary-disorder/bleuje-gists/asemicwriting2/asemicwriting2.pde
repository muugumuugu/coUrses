...
void move(){
  x *= 2*noise.eval(0.05*t,0);
  y *= 2*noise.eval(123+0.05*t,0);
...
void step(){
  ...
  float rot = map((float)noise.eval(0.05*px,0.05*py),-1,1,-TWO_PI,TWO_PI);
  
  rotate(0.1*rot);
  
  float xx = map(1.0*x,-1.1,1.1,-20,20)+0.5*randomGaussian();
  float yy = map(0.3*y,-1.1,1.1,-20,20)+0.5*randomGaussian();
  
  px += 0.0001 + 0.0003*(1+noise.eval(123+0.05*px,py)/2);
