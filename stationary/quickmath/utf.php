...
{
  return preg_replace_callback('/(&(?:(lt|gt|amp|quot|apos)|[a-zA-Z0-9]+);)/',
			'_convert2numbered',
			$content);
}

function name2utf8($content)
{
  return preg_replace_callback('/(&(?:(lt|gt|amp|quot|apos)|[a-zA-Z0-9]+);)/',
			'_convert2utf8',
			$content);
...
function _convert2numbered($reference)
{
  global $entities;
  if (count($reference) > 2)
    return $reference[1];
  preg_match('/^&([a-zA-Z0-9]+);$/',$reference[1],$matches);
  if (array_key_exists($matches[1],$entities))
    {
...
function _convert2utf8($reference)
{
  global $entities;
  if (count($reference) > 2)
    return $reference[1];
  ...
  if (array_key_exists($matches[1],$entities))
    {
      return html_entity_decode($entities[$matches[1]], ENT_QUOTES, 'UTF-8');
    }
  else
    {
       return "&amp;$matches[1];";
...
function get_unicode()
{
  ...
  request.onreadystatechange = _addtolist;
  ...
  if(request.readyState == 4 && request.status == 200)
    {
      ...
      listNode.innerHTML = request.responseText;
...
// ]]>
  ...
  <body>
  <p>
  <form id="utfform" action="utf.php" method="post">
  LaTeX Code: <input id="tex" name="tex" type="text" />
  <input id="submit" name="submit" value="Submit" type="submit" />
  <input id="ajax" name="ajax" value="Ajax" type="submit" onclick="javascript:get_unicode(); return false;" />
  </form>
  </p>
  <p>
  Results:
...
<?php
  ...
  if (array_key_exists("tex",$_REQUEST))
    {
      if (!$ajax_mode)
	{
?>
...
<?php
      ...
      if (preg_match('/^\\\\(.*){([A-Za-z])}$/',$tex, $matches))
	{
	  if (array_key_exists($matches[1],$alphabets))
	    ...
	    {
	      $error = "(not currently recognised)";
	    }
	}
      ...
      else
	{
	  switch ($output_mode) {
	    case 'utf8':
	      print name2utf8($output);
	      break;
	    case 'number':
	      print name2numbered($output);
	      break;
	    default:
	      print $output;
	    }
